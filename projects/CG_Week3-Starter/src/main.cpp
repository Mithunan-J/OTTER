...
std::string windowTitle = "Mithunan_Jayaseelan_100787442";
...
int main() {
	...
	static const float interleaved2[] = {
		// X     Y     Z       R     G     B
		 0.7f, -0.3f, 0.5f,   0.0f, 0.0f, 0.0f,
		 0.7f,  0.7f, 0.5f,   0.3f, 0.2f, 0.5f,
		-0.3f,  0.7f, 0.5f,   1.0f, 1.0f, 0.0f,
		-0.3f, -0.3f, 0.5f,   1.0f, 1.0f, 1.0f
	};
	VertexBuffer* interleaved_vbo2 = new VertexBuffer();
	...
	IndexBuffer* interleaved_ibo2 = new IndexBuffer();
	...
	shader2->LoadShaderPartFromFile("shaders/frag_shader_two.glsl", ShaderPartType::Fragment);
	shader2->Link();
	//end new shader//
	...
	while (!glfwWindowShouldClose(window)) {
		...
		glDrawElements(
			GL_TRIANGLES,
			(GLenum)interleaved_ibo2->GetElementCount(),
			(GLenum)interleaved_ibo2->GetElementType(), nullptr);
		VertexArrayObject::Unbind();
		//end new shape//
